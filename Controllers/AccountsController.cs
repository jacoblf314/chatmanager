...
namespace ChatManager.Controllers
{
    public class AccountsController : Controller
    {
        ...
        public ActionResult ExpiredSession()
        {
            ...
            if (userId != 0)
            {
                LoginEntry latestLoginEntry = DB.LoginEntry.ToList()
                    .Where(entry => entry.UserId == userId && entry.IsOnline)
                    .OrderByDescending(entry => entry.LoginTime)
                    .FirstOrDefault();

                if (latestLoginEntry != null)
                {
                    latestLoginEntry.IsOnline = false;
                    latestLoginEntry.Expired = true;
                    DB.LoginEntry.Update(latestLoginEntry);
                }
            }
        ...
        public ActionResult Logout()
        {
            ...
            if (userId != 0)
            {
                ...
                if (latestLoginEntry != null)
                {
                    latestLoginEntry.LogoutTime = DateTime.Now;
                    latestLoginEntry.IsOnline = false;
                    DB.LoginEntry.Update(latestLoginEntry);
                }
            }
        ...
        public ActionResult GetLoginsJournal(bool forceRefresh = false)
        {
            ...
            {
                return PartialView(DB.LoginEntry.ToList());
            }
            return null;
        }
        #endregion
        ...
        [OnlineUsers.AdminAccess]
        public void DeleteLoginEntries(string dateString)
        {
            DateTime date;
            DateTime.TryParse(dateString, out date);
            var entriesToDelete = DB.LoginEntry.ToList().Where(entry => entry.LoginTime.Date == date);

            foreach (var entry in entriesToDelete)
            {
                DB.LoginEntry.Delete(entry.Id);
            }
